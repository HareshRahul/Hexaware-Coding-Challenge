        ...
        static void Main(string[] args)
        {
            ...
            while (!exit)
            {
                try
                {
                    ...
                    Console.WriteLine("8. Create Claim");
                    Console.WriteLine("9. View Claims by Policy ID");
                    ...
                    switch (input)
                    {
                        case "1":
                            ...
                            if (!availablePolicyNames.Contains(pname))
                            {
                                Console.WriteLine("Invalid Policy Name. Choose from the list.");
                                break;
                            }
                            ...
                            if (!availablePolicyTypes.Contains(ptype))
                            {
                        ...
                        case "3":
                            ...
                            if (int.TryParse(Console.ReadLine(), out int searchId))
                            {
                                ...
                                if (found != null)
                                {
                                    Console.WriteLine($"ID: {found.PolicyId}, Name: {found.PolicyName}, Type: {found.PolicyType}, Coverage: {found.CoverageAmount}");
                                }
                                else
                                {
                        ...
                        case "4":
                            ...
                            if (int.TryParse(Console.ReadLine(), out int upId))
                            {
                                ...
                                if (!availablePolicyNames.Contains(newName))
                                {
                                ...
                                if (!availablePolicyTypes.Contains(newType))
                                {
                                ...
                                if (decimal.TryParse(Console.ReadLine(), out decimal newAmt))
                                {
                                    ...
                                    Console.WriteLine(result ? " Policy updated." : " Policy update failed.");
                                }
                                else
                                {
                                    Console.WriteLine("Invalid amount.");
                        ...
                        case "8":
                            Console.Write("Enter Policy ID for claim: ");
                            if (int.TryParse(Console.ReadLine(), out int clPolicyId))
                            {
                                Console.Write("Enter Claim Reason: ");
                                string reason = Console.ReadLine();

                                Console.Write("Enter Claim Amount: ");
                                if (decimal.TryParse(Console.ReadLine(), out decimal clAmt))
                                {
                                    Claim cl = new Claim
                                    {
                                        policyId = clPolicyId,
                                    ...
                                    bool claimStatus = service.CreateClaim(cl);
                                    Console.WriteLine(claimStatus ? "Claim filed." : " Failed to file claim.");
                                }
                                else
                                {
                                    Console.WriteLine("Invalid amount.");
                        ...
                        case "9":
                            Console.Write("Enter Policy ID to view claims: ");
                            if (int.TryParse(Console.ReadLine(), out int viewPid))
                            {
                                List<Claim> claims = service.GetClaimsByPolicyId(viewPid);
                                Console.WriteLine($"\nClaims for Policy ID {viewPid}:");
                                foreach (var cl in claims)
                                {
                                    Console.WriteLine($"Claim ID: {cl.claimId},Amount: {cl.claimAmount}");
                                }
                            }
                            else
                            {
